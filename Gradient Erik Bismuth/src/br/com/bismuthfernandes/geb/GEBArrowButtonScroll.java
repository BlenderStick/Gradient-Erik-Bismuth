 ...
 *
 */
public class GEBArrowButtonScroll extends JButton {
	/**
	 * 
	 */
	...
	public GEBArrowButtonScroll(int orientation, int mirror){
	...
	private void createPath(){
		if(mirror<-1 || mirror>1)
			throw new IllegalArgumentException("mirror precisa ser -1, 0 ou 1");
		Path2D p = new Path2D.Double();
		double w=0;// = getWidth();
		double h=0;// = getHeight();
		if(orientation==SwingConstants.NORTH||orientation==SwingConstants.SOUTH){
			w = getWidth()+1;h=getHeight();
		} else {
			w = getHeight();h=getWidth()+1;
		}
...
//			p.lineTo(w, h/2);
		...
		if (orientation==SwingConstants.NORTH){
			...
			af.translate(0, -getHeight());
			P = new Path2D.Double(p.createTransformedShape(af));
			
		} else if(orientation==SwingConstants.EAST){
			AffineTransform af = new AffineTransform();
//			af.rotate(Math.toRadians(90), w/2, h/2);
//			af.translate(2, 2);
			af.rotate(Math.toRadians(90), 0, 0);
			af.translate(0, -getHeight()+4);
			P = new Path2D.Double(p.createTransformedShape(af));
